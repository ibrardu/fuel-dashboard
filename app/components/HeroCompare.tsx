...
export default function HeroCompare({
  ...
  const { waterfall } = activeCity;

  const freightDealer = waterfall.freight + waterfall.dealer;
  const segments = [
    { label: "Base (OMC realisation)", value: waterfall.base, cls: "bg-emerald-500" },
    { label: "Freight + dealer", value: freightDealer, cls: "bg-slate-500" },
    { label: "Central excise", value: waterfall.excise, cls: "bg-slate-600" },
    { label: "State VAT", value: waterfall.vat, cls: "bg-slate-700" },
  ];
      ...
      <div className="flex flex-wrap items-end justify-between gap-x-6 gap-y-4">
        ...
        <div>
          ...
          <div className="mt-1 text-[11px] text-emerald-400/70">
            what the E{blendPct} feedstock actually costs OMCs
          </div>
        </div>
      </div>
      ...
      <div className="mt-7">
        <div className="mb-2 flex items-baseline justify-between">
          <span className="text-xs font-medium text-slate-400">
            Where your ₹{activeCity.quoted.toFixed(2)} goes
          </span>
          <span className="text-[11px] tabular-nums text-slate-500">₹/L, {activeCity.name} build-up</span>
        </div>
        <div className="flex h-4 w-full gap-[2px] overflow-hidden rounded-full">
          {segments.map((s) => (
            <div
              ...
              style={{ width: `${(s.value / activeCity.quoted) * 100}%` }}
        ...
        <p className="source">
          Taxes and levies are ₹{(waterfall.excise + waterfall.vat).toFixed(2)}/L of the pump
          price. The ₹{waterfall.base.toFixed(2)} base is charged as petrol-equivalent — the
          blended feedstock inside it costs ₹{nationalBlendedCost.toFixed(2)} with ethanol at ₹
          {ethanolPrice.toFixed(2)}/L, and pump prices did not move when the blend did.
        </p>
      </div>
    </div>
  );
